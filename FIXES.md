...
Working tree pins **`FROM cartagodocker/zsh:v2.0.0`** (Hub republished
with `dockerzsh eza -v` → `v0.18.2`). Local smoke on
`nodebun-local:dev` (~695 MB) passed: no ENTRYPOINT,
...
(`~/.local/state` + chown), login zsh `su -` via `/etc/zsh/zprofile`
sourcing `nodebun.sh` (baked `NODE_DEFAULT_VERSION`), catalogue
`dockernodebun`. Do not retag Hub zsh `v1.0.5`.
